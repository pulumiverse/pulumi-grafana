// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.grafana.outputs;

import com.pulumi.core.annotations.CustomType;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;

@CustomType
public final class GetTeamPreference {
    private @Nullable String homeDashboardUid;
    private @Nullable String theme;
    private @Nullable String timezone;

    private GetTeamPreference() {}
    public Optional<String> homeDashboardUid() {
        return Optional.ofNullable(this.homeDashboardUid);
    }
    public Optional<String> theme() {
        return Optional.ofNullable(this.theme);
    }
    public Optional<String> timezone() {
        return Optional.ofNullable(this.timezone);
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(GetTeamPreference defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private @Nullable String homeDashboardUid;
        private @Nullable String theme;
        private @Nullable String timezone;
        public Builder() {}
        public Builder(GetTeamPreference defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.homeDashboardUid = defaults.homeDashboardUid;
    	      this.theme = defaults.theme;
    	      this.timezone = defaults.timezone;
        }

        @CustomType.Setter
        public Builder homeDashboardUid(@Nullable String homeDashboardUid) {
            this.homeDashboardUid = homeDashboardUid;
            return this;
        }
        @CustomType.Setter
        public Builder theme(@Nullable String theme) {
            this.theme = theme;
            return this;
        }
        @CustomType.Setter
        public Builder timezone(@Nullable String timezone) {
            this.timezone = timezone;
            return this;
        }
        public GetTeamPreference build() {
            final var o = new GetTeamPreference();
            o.homeDashboardUid = homeDashboardUid;
            o.theme = theme;
            o.timezone = timezone;
            return o;
        }
    }
}
