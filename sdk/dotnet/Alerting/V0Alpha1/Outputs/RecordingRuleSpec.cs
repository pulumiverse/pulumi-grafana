// *** WARNING: this file was generated by pulumi-language-dotnet. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;
using Pulumi;

namespace Pulumiverse.Grafana.Alerting.V0Alpha1.Outputs
{

    [OutputType]
    public sealed class RecordingRuleSpec
    {
        /// <summary>
        /// A sequence of stages that describe the contents of the rule. Each value is a JSON string representing an expression object.
        /// </summary>
        public readonly ImmutableDictionary<string, string> Expressions;
        /// <summary>
        /// Key-value pairs to attach to the recorded metric.
        /// </summary>
        public readonly ImmutableDictionary<string, string>? Labels;
        /// <summary>
        /// The name of the metric to write to.
        /// </summary>
        public readonly string Metric;
        /// <summary>
        /// Sets whether the recording rule should be paused or not.
        /// </summary>
        public readonly bool? Paused;
        /// <summary>
        /// The UID of the datasource to write the metric to.
        /// </summary>
        public readonly string TargetDatasourceUid;
        /// <summary>
        /// The title of the recording rule.
        /// </summary>
        public readonly string Title;
        /// <summary>
        /// The trigger configuration for the recording rule.
        /// </summary>
        public readonly Outputs.RecordingRuleSpecTrigger? Trigger;

        [OutputConstructor]
        private RecordingRuleSpec(
            ImmutableDictionary<string, string> expressions,

            ImmutableDictionary<string, string>? labels,

            string metric,

            bool? paused,

            string targetDatasourceUid,

            string title,

            Outputs.RecordingRuleSpecTrigger? trigger)
        {
            Expressions = expressions;
            Labels = labels;
            Metric = metric;
            Paused = paused;
            TargetDatasourceUid = targetDatasourceUid;
            Title = title;
            Trigger = trigger;
        }
    }
}
